  ...
  <div>
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb small mb-1">
        ...
        <li class="breadcrumb-item active"><?= t('superuser.title') ?></li>
      </ol>
    </nav>
        ...
        <tbody>
          ...
          <tr class="<?= $impersonating === $tenantId ? 'table-primary' : '' ?>">
            ...
            <td>
              ...
              <?php else: ?>
                <?php foreach ($group['subUsers'] as $sub): ?>
                  <span class="badge bg-secondary-subtle text-secondary-emphasis rounded-pill me-1"><?= e($sub['name']) ?></span>
                <?php endforeach; ?>
              <?php endif; ?>
            </td>
            <td class="text-end">
              <form method="post" action="/superuser/impersonate" class="m-0">
                <?= csrf_field() ?>
