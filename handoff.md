...
### 4.128 Sidebar — ცარიელი პუნქტები წაშლილია

user: „left bar-ის მენიუში თავისუფალი მენიუ ვნახოთ და წავშალოთ."

Ორი პუნქტი `#`-ზე იდგა — placeholder-ები დაუწერელი გვერდებისთვის:
`nav.analytics_reports` („რეპორტები") და `nav.orders_pending`
(„მოლოდინში"). Ორივე წაშლილია `menu.json`-იდან, ენის ფაილებიდანაც.

Რეპორტების წაშლის შემდეგ „ანალიტიკა" ჯგუფს **ერთი** შვილი დარჩა
(მიმოხილვა) — ერთშვილიანი dropdown არაფრისთვის იხსნება, ამიტომ ჯგუფი
პირდაპირ ბმულად ჩამოიშალა: `nav.analytics → /analytics/overview`.
„შეკვეთები" ჯგუფად რჩება (ყველა / ახალი).

**გადამოწმებულია ცოცხლად**: sidebar-ში `href="#"` — **0**; „ანალიტიკა"
`<a href="/analytics/overview">` და `/analytics/overview`-ზე `active`;
დაუმუშავებელი გასაღები — 0.


### 4.129 InvoiceWorkflow ცოცხალი აპლიკაციიდან ამოღებულია — workshop-ში რჩება

user: „ინვოისის სტატუსის გაფართოება სისტემიდან ამოვშალოთ (uninstall),
შემდგომი დამუშავებისთვის modules.loc-ში გადავიდეს."

Workshop-ში ის უკვე იყო (`4.113`-დან, v2.1.0, `ModuleDb`-ზე) —
შედარება: შინაარსობრივად იდენტური (ერთი „Run with" კომენტარი
ლოკაციის მიხედვით განსხვავდება, სწორად). Ანუ „გადატანა" = ცოცხალიდან
ამოღება.

**Live-ის მდგომარეობა ამოღებამდე**: დაინსტალირებული (v2.0.0 registry-ში
— `2.1.0` module.json-ში, install ხელახლა არ გაშვებულა), **არავისთვის
ჩართული**, `invoice_workflow` ცარიელი.

**Ამოღება — იმავე სამი ნაბიჯით, რასაც `ModuleController` აკეთებს**:
1. `ModuleArchive::exportData()` →
   `storage/backups/InvoiceWorkflow_2026-09-17_014647.sql` (სქემა, 0 row)
2. `ModuleRegistry::uninstall()` → ცხრილი წაშლილი, `modules` /
   `module_tenants` / `migrations`-ledger — 0 row
3. `ModuleArchive::removeFiles()` → `app/Modules/InvoiceWorkflow/` წაშლილი,
   `discover()` → `[]`

`/help/modules`-ის „ცოცხალი ნიმუში" ხაზი ახლა workshop-ზე მიუთითებს
(`help.mod_reference`, ka/en, workshop-შიც სინქრონირებული).

**გადამოწმებულია ცოცხლად** (SuperUser → tenant 1): `/`, `/orders`,
`/invoices?edit=59`, `/settings/modules`, `/help/modules` — 200, სუფთა;
მოდულების გვერდი ცარიელ მდგომარეობაშია; `/orders`-ზე მოდულის კვალი
(`invoiceworkflow`, „ნაწილობრივ") — არაა; „მოდულები" მენიუ არ ჩანს.
Ბირთვის hook-წერტილები ადგილზეა — უბრალოდ მსმენელი აღარ ჰყავთ.

Ცოცხალ აპლიკაციაში ახლა **ნული მოდულია** — სწორედ ისე, როგორც
საწარმოო სისტემა უნდა იყოს, სანამ მოდული workshop-ში არ დასრულდება და
ZIP-ად არ მოვა.


### 4.130 `/help` — მთელი სისტემის ფუნქციონალის ინდექსი

user: „შევქმნათ მთლიანი სისტემის დახმარების გვერდი — ფუნქციონალის
ჩამონათვალი ბმულებით."

`/help` აქამდე ერთბარათიანი იყო (მოდულის შექმნა). Ახლა **7 სექცია,
26 პუნქტი**, თითოეული: სახელი, ერთხაზიანი აღწერა, პირდაპირი ბმული:

| სექცია | პუნქტები |
|---|---|
| სამუშაო მაგიდა და ანალიტიკა | დეშბორდი, მიმოხილვა |
| ინვოისები | ახალი, ყველა, ფილტრები, PDF, ნახვა, მეილი, გაზიარება, დუბლირება |
| დამკვეთები და პროდუქცია | დამკვეთები, რეპორტი, პროდუქცია |
| პარამეტრები | ორგანიზაცია, მომხმარებლები, მოდულები, პროფილი ×2, დიზაინ-სისტემა |
| ანგარიში და შესვლა | პაროლი, OTP, Google, აღდგენა |
| SuperUser | მართვა, აქტივობის ლოგი — **მხოლოდ superadmin-ისთვის** |
| დეველოპერებისთვის | მოდულის შექმნა |

**Data-driven** — ერთი მასივი view-ში, ახალი ფუნქცია = ერთი ხაზი.
SuperUser-სექცია იმავე `role === 'superadmin'` პირობით, რასაც sidebar
იყენებს — tenant-ის ადმინი ვერ ხედავს ბმულებს, რომლებსაც ვერ გახსნის.
Განზრახ **სია და არა სახელმძღვანელო**: გვერდები თავად ხსნიან თავს;
ინდექსისგან ადამიანს სჭირდება „აკეთებს თუ არა X-ს და სად".

Ბარათები `card-header`-ით (`4.124`-ის სტილი), `list-group-flush`,
`stretched-link` — მთელი მწკრივი დაკლიკებადია.

**Workshop-ში** `help/index.php` **არ** გადადის — იქ ჩამოთვლილი
ფუნქციების უმეტესობა არ არსებობს. `modules.loc/CLAUDE.md`-ის refresh-
სია ახლა `app/Views/help/modules.php`-ს ასახელებს, არა მთელ საქაღალდეს.
Lang სინქრონირებულია (ზედმეტი გასაღები უვნებელია).

**გადამოწმებულია ცოცხლად** (SuperUser → tenant 1, `1400×900`): 7
სექცია, 26 პუნქტი, 17 უნიკალური ბმული — **ყველა 200, მკვდარი 0**;
დაუმუშავებელი გასაღები 0; SuperUser-სექცია ჩანს (superadmin-ით).
ka/en **674 = 674**.


### 4.131 დიზაინ-სისტემის გვერდი workshop-შია — ცოცხალიდან ამოღებულია

user: „მენიუ დიზაინის სისტემა — ესეც modules.loc-ში, აქ აღარაა საჭირო."

Სწორია: ყველა კომპონენტი და ტოკენი ერთ გვერდზე მოდულის ავტორს
სჭირდება მშენებლობისას, კლიენტს — არასდროს.

**Workshop-ში დაემატა**: `StyleGuideController`, `Views/style-guide.php`,
მარშრუტი `/style-guide`, მენიუ `nav.system`-ის თავში.

**Ცოცხალიდან წაიშალა**: კონტროლერი, view, მარშრუტი + import, მენიუს
პუნქტი, `/help`-ის ჩანაწერი, `ActivityLog`-ის ეტიკეტი,
`help.f_style_guide*` და `superuser.act_style_guide` გასაღებები.

**Ლანგ-გასაღებები, რომლებიც განზრახ დარჩა** ცოცხალის ენის ფაილებში:
`nav.design_system`, `page.style_guide`, `sg.*` (35 ცალი). Მიზეზი:
workshop-ის `app/lang/` ცოცხალის **ასლია** (refresh-პროცედურით) — თუ
აქედან წაიშლება, workshop-ის გვერდი შემდეგ სინქრონზე დაუმუშავებელ
გასაღებებს აჩვენებს. 35 გამოუყენებელი სტრიქონი ცოცხალში იაფია;
lang-ის ორ წყაროდ გაყოფა — არა.

**გადამოწმებულია ცოცხლად** — live: `/style-guide` → **404**, sidebar-ში
და `/help`-ში ბმულა არაა, help 25 პუნქტი, გვერდები სუფთა. Workshop:
`/style-guide` → 200, სათაური სწორი, sidebar-ში ბმული `active`,
დაუმუშავებელი `sg.*` — 0. ka/en **671 = 671**.

Ერთი ცრუ განგაში გზაში: ბრაუზერის შემოწმებამ workshop-ზე „Fatal error"
დააბრუნა — გვერდი ჯერ არ იყო ჩატვირთული (`title: ""`); 500ms
დაყოვნებით — სუფთა. curl-ი თავიდანვე 200-ს იძლეოდა.


### 4.132 Cross-tenant IDOR დახურულია — სამი გზა

user-მა სთხოვა „თვალი გადაავლე, რამე ხომ არ გამომრჩა". Გამომრჩა
სესიის დასაწყისში აღმოჩენილი **IDOR** — ცალკე სესიის სამუშაო არ
ჩამოსულა (git-ში არცერთი კომიტი). Შემოწმებისას **მესამე** გზაც
გამოჩნდა.

| გზა | იყო | ახლა |
|---|---|---|
| `GET /invoices?edit=N` | `Invoice::find()` შემოწმების გარეშე — სხვისი ინვოისი ფორმაში | `ownerTenant() !== $ruler` → როგორც არარსებული (ცარიელი ფორმა, hidden id არა) — `loadDuplicateOld()`-ის იგივე არჩევანი, id-ების არსებობა ვერ იზომება |
| `POST /invoices` + `invoice_id` | პირდაპირ `save()` → `UPDATE … WHERE id = ?` | ვალიდაციამდე `find()` + `ownerTenant()`; უცხო → **404**, როგორც `show()`/`preview()` |
| `customerInvoicesPanel` `data-invoices` | `Invoice::all()` **არგუმენტის გარეშე = ყველა tenant-ის ინვოისი**, serialized ატრიბუტში | `Invoice::all(Auth::invoiceScopeUserIds())` |

Მესამე ყველაზე მზაკვრულია: პანელი უცხო მონაცემს **არასდროს აჩვენებდა**
(დამკვეთები tenant-scoped-ია, უცხო customer_id ვერ აირჩევა) — მაგრამ
გვერდის წყაროში მთელი სისტემის ინვოისების ნომრები და ჯამები იდო,
ნებისმიერი tenant-ისთვის. Ვიზუალურად უხილავი, `view-source`-ით სრული.

`ownerTenant()` helper-ი უკვე არსებობდა და 4 ადგილას სწორად იყო
გამოყენებული — ეს ორი გზა უბრალოდ არ ეძახდა.

**გადამოწმებულია ცოცხლად, ნამდვილი cross-tenant ტესტით** — tenant 31
(`test1@invoice.net`, დროებითი პაროლით) tenant 1-ის ინვოისზე (id 59,
438.90 ₾):
- `?edit=59` → ცარიელი „ახალი ინვოისი" ფორმა, `invoice_id` hidden არაა,
  `438.90` გვერდზე **არაა**
- `POST invoice_id=59` → **404**; ინვოისი 59 უცვლელი (438.90, customer 1545)
- პანელის `data-invoices` → მხოლოდ საკუთარი დამკვეთი (1553); უცხო id/ჯამი
  **არაა**
- საკუთარი ინვოისი (id 12) რედაქტირებისთვის ნორმალურად იტვირთება

Tenant 31-ის ჰეში აღდგენილია.


### 4.133 `units` tenant-scoped — ბოლო საერთო lookup-ცხრილი

`4.132`-ის სვიპმა (`find($id)` მფლობელობის გარეშე, ყველა მოდელზე)
ერთი გამონაკლისი იპოვა: **`units`-ს `ruler` არ ჰქონდა**. 003-ის 6
ერთეული (ცალი, მეტრი, კგ…) სამივე tenant-ს საერთო ჰქონდა, და
`POST /units` + `id=1&name=X` **ნებისმიერ** შესულ მომხმარებელს
„ცალი"-ს ყველასთვის გადაარქმევინებდა — 29 ინვოისის პოზიციაზე, 17
პროდუქტზე, PDF-ებზე. user: „units-ს ruler სჭირდება — ერთ
მომხმარებელს შეიძლება სჭირდებოდეს, მეორეს არა."

**`migrations/039`** — `ruler INT UNSIGNED NULL`, 023-ის ზუსტი
პრეცედენტით, ერთი დამატებით: **`NULL` = საერთო default**. 003-ის 6
ერთეული NULL რჩება — ყველა ხედავს, ვერავინ არქმევს, არსებული 29+17
მითითება ხელუხლებელია (არც per-tenant ასლები, არც remap). Tenant-ის
ახალი ერთეული მის ruler-ს ატარებს და მხოლოდ მისია.

- `Unit::all($ruler)` → `WHERE ruler IS NULL OR ruler = ?`;
  `create($name, $ruler)`; `update($id, $name, $ruler): int` (rowCount).
- `ProductType::update()` ასევე rowCount-ს აბრუნებს.
- **`LookupController`** — ორი თითქმის იდენტური გზა (`productTypes()`
  ruler-ით, `save()` ruler-ის გარეშე) **ერთად**: განსხვავების ერთადერთი
  მიზეზი units-ის ruler-ის არქონა იყო. Ახლა `update() === 0` → **404** —
  ადრე უცხო id-ზე `{id, name}` წარმატებას აბრუნებდა (ცრუ წარმატება,
  product-types-ზეც).
- `Unit::all()`-ის 2 გამომძახებელი (`Invoice`/`ProductController`) ruler-ს
  გადასცემს.

**გადამოწმებულია ცოცხლად, tenant 31-ით**: shared `id=1` → „HACKED" →
**404**, „ცალი" უცვლელი; საკუთარი შექმნა → id 7 (`ruler 31`); საკუთარის
გადარქმევა → 200; უცხო id → 404. `Unit::all(1)` → 6 (unit 7 **არ ჩანს**),
`Unit::all(31)` → 7. Default-ები 6/6 ხელუხლებელი. Unit 7 წაშლილი, 31-ის
ჰეში აღდგენილი. Workshop სინქრონირებული + მიგრირებული, hook host
იდენტური.
...
### 4.135 ფასდაკლება ინვოისზე

user-ის იდეა, ჩემი რეკომენდაციით ჩამოყალიბებული: **ინვოისის დონეზე**
(არა პოზიციაზე), **პროცენტი ან თანხა** ერთ ველში გადამრთველით.

**Სამი დიზაინის გადაწყვეტილება**:
1. Ინვოისის დონე — ~90% შემთხვევა („მთლიანზე 10%"), 2 სვეტი, ერთი ველი,
   PDF-ზე 2 ხაზი. Პოზიციური მოგვიანებით, თუ დასჭირდა.
2. `discount_type` (`percent`|`amount`) + `discount_value` — ინახება ის,
   რაც **შეთანხმდა** („10%" ან „100 ₾"), არა გადათვლილი ეკვივალენტი.
3. Დღგ — ფასები VAT-inclusive-ია, ანუ ფასდაკლება უბრალოდ ჯამს ამცირებს,
   საინფორმაციო დღგ-ხაზი ახალი ჯამიდან გამოითვლება. „დღგ-მდე თუ შემდეგ"
   კითხვა **არ დგება**.

**Migration 040**: `discount_type ENUM DEFAULT 'percent'`, `discount_value
DECIMAL(12,2) DEFAULT 0` — არსებული 21 ინვოისი ნულოვანი ფასდაკლებით,
უცვლელი. `total` რჩება საბოლოო (ფასდაკლების შემდეგ) ჯამად, რასაც ყველა
სია/რეპორტი ისედაც კითხულობს.

**Ერთი ფორმულა** — `Invoice::applyDiscount($subtotal, $type, $value)` +
`Invoice::subtotal($items)`: `save()` ამით წერს `total`-ს, JS ფორმაში
იმავეს ასახავს (მხოლოდ ჩვენებისთვის — სერვერი შენახვისას თავად
გადაითვლის), preview/PDF/share-გვერდი `subtotal − total`-ით ღებულობს
ჩამოწერილ თანხას. Ვერსად ვერ დაშორდება.

**Ვალიდაცია**: ცარიელი = 0; არაუარყოფითი რიცხვი; პროცენტი ≤100; თანხა ≤
subtotal (პოზიციების გაცნობის შემდეგ). 100% დაშვებულია — `is_zero`
ფლაგს ავტომატურად არ ვცვლი.

**Ფორმა**: ჯამის ბლოკში „ჯამი ფასდაკლებამდე" → ფასდაკლების input +
`%`/`₾` btn-group → „ფასდაკლება: − X" (მხოლოდ >0-ზე) → დღგ → ჯამი.
`.ds-discount-group` CSS (input-group 100%-ს იკავებდა). Რედაქტირება და
დუბლირება (`$old`) ფასდაკლებას ატარებს.

**Ჩვენება 4 ადგილას** (ფორმა, preview, PDF, share) — ფასდაკლების ორი
ხაზი მხოლოდ მაშინ, როცა არსებობს; უფასდაკლებო ინვოისი ზუსტად ისე
გამოიყურება, როგორც ადრე.

**გადამოწმებულია ცოცხლად, tenant 31-ით** (`1400×900`):
- JS: 2×100 → 200.00; 10% → −20.00, დღგ 27.46 (=180×18/118), ჯამი 180.00;
  ₾25 → −25.00, 175.00, ღილაკი `active`
- შენახვა 10% → DB: `total 180.00, percent, 10.00`; 21 ძველი ინვოისი
  `0/percent` — უცვლელი
- ვალიდაცია: 101% / −5 / თანხა>ჯამზე → სამივე შესაბამისი შეცდომით
- რედაქტირება: prefill `10`/`percent`, `%` active, ცოცხალი ჯამები სწორი
- preview: 4 ხაზი; share: 3 ხაზი; დუბლირება: prefill ატარებს;
  ინვოისი 12 (უფასდაკლებო): ფასდაკლების ხაზი **არაა**
- PDF: 200 OK, `%PDF`, შაბლონში ხუთივე ფიგურა
- ტესტ-ინვოისი 61 წაშლილი, ჰეში აღდგენილი; workshop სინქრონირებული
  (hook host-ები იდენტური), InvoiceWorkflow 13/13


### 4.136 „ნულოვანი" → „დღგ-ს გამოჩენა" + დამკვეთის ინვოისები ბმულებად

**Გადამრთველი**. `invoices.is_zero` → `show_vat` (migration 041: `CHANGE
COLUMN` + `UPDATE SET show_vat = NOT show_vat` — ძველი „ნულოვანი=0"
ინვოისები ავტომატურად „დღგ ჩანს"-ზე გადავიდა, 22-ივე ინვოისი `1`).
Ახალ ფორმაზე ჩართულია (`$old === []`), წარუმატებელი submit-ის/რედაქტირების
შემდეგ — რაც დაპოსტდა/ინახება (checkbox POST-ში არ არის, როცა
გამორთულია → `isset`). `Invoice::validate()` → `'show_vat' => isset(...) ?
1 : 0`; `$old` prefill რედაქტირებაშიც და დუბლირებაშიც.

**Სად მოქმედებს**: ფორმის ჯამში `#invoiceVatRow` (`syncVatRow()` JS),
`invoice-preview.php`, `pdf/invoice.php` — დღგ-ის ხაზი `<?php if
($invoice['show_vat'])`. Share-გვერდზე (`invoice-view.php`) დღგ-ხაზი
არასდროს ყოფილა — უცვლელი. **Ჯამს არ ცვლის**: ფასები VAT-inclusive-ია,
გადამრთველი მხოლოდ იმას წყვეტს, გამოჩნდეს თუ არა გამოყოფილი დღგ.

**Დამკვეთის ინვოისების პანელი**: `$invoicesByCustomer[..][]`-ს დაემატა
`'id'`; JS თითოეულ ხაზს `<a href="/invoices?edit=${parseInt(inv.id,10)}"
class="… ds-invoice-link">`-ად აწყობს (href მხოლოდ int-იდან, არა ტექსტიდან).
`?edit=N` უკვე ownership-guarded-ია (4.132), ამიტომ ბმულით უცხო
ინვოისზე გადასვლა შეუძლებელია. `.ds-invoice-link:hover` — ფონი
`--bs-tertiary-bg`.

**Ენა**: `inv.flag_zero` წაშლილია, `inv.show_vat` = „დღგ-ს გამოჩენა" /
„Show VAT". Parity ka=en=676.

**გადამოწმებულია ცოცხლად, tenant 31-ით**:
- ახალი ფორმა: ლეიბლი „დღგ-ს გამოჩენა", ჩართული, დღგ-ხაზი ჩანს;
  გამორთვა → ხაზი ქრება, ჯამი უცვლელი
- ინვოისი 63 (გამორთული): preview-ში დღგ **არაა**, PDF-შაბლონში
  **არაა**, რედაქტირებაზე გადამრთველი გამორთული; ინვოისი 64 (ჩართული):
  სამივეგან **არის**; PDF ორივე 200
- პანელი: `/invoices?edit=12` ბმული, hover-ფონი
- ტესტ-ინვოისები 63/64 წაშლილი, ჰეში აღდგენილი; workshop სინქრონირებული
  (041 გატარებული, hook host-ები იდენტური), InvoiceWorkflow 13/13
